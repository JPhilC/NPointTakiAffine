...
namespace NPoint
{
	internal static class EQMath
	{
		...
		public const double DEG_RAD = 0.0174532925d;
		public const double RAD_DEG = 57.2957795d;
		public const double HRS_RAD = 0.2617993881d;
		public const double RAD_HRS = 3.81971863d;

		public const double SID_RATE = 15.041067d;
		public const double SOL_RATE = 15;
		public const double LUN_RATE = 14.511415d;

		public const double gEMUL_RATE = 20.98d; // 0.2 * 9024000/( (23*60*60)+(56*60)+4)
		// 0.2 = 200ms

		public const double gEMUL_RATE2 = 104.730403903004d; // (9024000/86164.0905)

		// 104.73040390300411747513310083625

		public const double gARCSECSTEP = 0.144d; // .144 arcesconds / step

		// Iterative GOTO Constants
		//Public Const NUM_SLEW_RETRIES As Long = 5                   ' Iterative MAX retries
		public const double gRA_Allowed_diff = 10; // Iterative Slew minimum difference
		...
		public const double gRAEncoder_Zero_pos = 0x800000; // ENCODER 0 Hour initial position
		public const double gDECEncoder_Zero_pos = 0x800000; // ENCODER 0 Degree Initial position

		public const double gDefault_step = 9024000; // Total Encoder count (EQ5/6)
		...
		internal static double Get_EncoderHours(double encOffset0, double encoderval, double Tot_enc, int hmspr)
		{
			...
			if (hmspr == 0)
			{
				return Range24(i + 6d); // Set to true Hours which is perpendicula to RA Axis
			}
			else
			{
				return Range24((24 - i) + 6d);
			}

		}

		internal static int Get_EncoderfromHours(double encOffset0, ref double hourval, double Tot_enc, int hmspr)
		{
		...
		internal static int Get_EncoderfromDegrees(double encOffset0, ref double degval, double Tot_enc, double Pier, int hmspr)
		{
		...
		internal static double Get_EncoderDegrees(double encOffset0, double encoderval, double Tot_enc, int hmspr)
		{
		...
		internal static double Range_DEC(double decdegrees)
		{
		...
		//internal static int Get_RAEncoderfromRA(double ra_in_hours, double dec_in_degrees, double pLongitude, double encOffset0, double Tot_enc, int hmspr)
		//{


		//	double i = ra_in_hours - EQnow_lst(pLongitude * DEG_RAD);
		...
		internal static int Get_RAEncoderfromAltAz(double Alt_in_deg, double Az_in_deg, double pLongitude, double pLatitude, double encOffset0, double Tot_enc, int hmspr)
		{
		...
		internal static int Get_DECEncoderfromAltAz(double Alt_in_deg, double Az_in_deg, double pLongitude, double pLatitude, double encOffset0, double Tot_enc, double Pier, int hmspr)
		{
			object[, , , , ] aa_hadec = null;
		...
		internal static int Get_DECEncoderfromDEC(double dec_in_degrees, double Pier, double encOffset0, double Tot_enc, int hmspr)
		{
		...
		internal static string printhex(double inpval)
		{
		...
		//internal static double EQnow_lst_time(double plong, double ptime)
		//{
		//	object mjd_hr = null;
		//	object mjd_day = null;
		//	double[] vb_mjd = null;
		...
		internal static PierSide2 SOP_DEC(double DEC)
		{
		...
		//internal static PierSide2 SOP_Physical(double vha)
		//{
		...
		//internal static PierSide2 SOP_Pointing(double DEC)
		//{
		...
		//	return result;
		//}
		//internal static PierSide2 SOP_RA(double vRA, double pLongitude)
		//{
		...
		internal static double Range24(double vha)
		{
		...
		internal static double Range360(double vdeg)
		{
		...
		internal static double Range90(double vdeg)
		{
		...
		internal static double RangeHA(double ha)
		{
		...
		internal static double GetSlowdown(double deltaval)
		{
		...
		internal static double Delta_RA_Map(double RAENCODER)
		{
		...
		internal static double Delta_DEC_Map(double DecEncoder)
		{
		...
		internal static Coordt Delta_Matrix_Map(double RA, double DEC)
		{
			...
			int i = EQTakiAffine.EQ_UpdateTaki(RA, DEC);

			Coord obtmp2 = EQTakiAffine.EQ_plTaki(obtmp);
		...
		internal static Coordt Delta_Matrix_Reverse_Map(double RA, double DEC)
		{
			...
			int i = EQTakiAffine.EQ_UpdateAffine(obtmp.x, obtmp.Y);
			Coord obtmp2 = EQTakiAffine.EQ_plAffine(obtmp);
		...
		internal static Coordt DeltaSync_Matrix_Map(double RA, double DEC)
		{
			Coordt result = new Coordt();
			int i = 0;
		...
		internal static Coordt DeltaSyncReverse_Matrix_Map(double RA, double DEC)
		{
			Coordt result = new Coordt();
			int i = 0;
		...
		internal static int GetQuadrant(Coord tmpcoord)
		{
			int ret = 0;
		...
		internal static int GetNearest(double RA, double DEC)
		{
			...
			tmpcoord.x = RA;
			tmpcoord.Y = DEC;
			tmpcoord = EQTakiAffine.EQ_sp2Cs(tmpcoord);
			...
			else
			{
				...
				i = EQTakiAffine.EQ_FindLowest(datholder, 1, Count);
				if (i == -1)
				{
					return -1;
				}
